...
public:
    ...
    virtual double Evaluate(std::function<double(const Position* cell)> args) const = 0;
...
public:
    ...
    double Evaluate(std::function<double(const Position* cell)> args) const override {
        switch (type_) {
            case Add:
...
double FormulaAST::Execute([[maybe_unused]] std::function<double(const Position* cell)> args) const {
